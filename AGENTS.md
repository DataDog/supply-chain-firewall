...
- `scfw/internal/proxy` owns the package-manager-independent interception lifecycle: ephemeral reverse proxying, request and response logging, synthetic responses, evaluation gating, and crash-safe child-process execution; `proxy/manager/<manager>` discovers and overrides configuration for npm, Yarn, pnpm, Bun, pip, Poetry, or uv, while `proxy/ecosystem/npm` and `proxy/ecosystem/pypi` rewrite ecosystem-specific metadata, identify downloads, and produce packages for evaluation.
