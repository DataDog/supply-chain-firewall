...
test: test-cli test-configure test-python-executable test-pip test-pip-class test-poetry test-poetry-class test-npm test-npm-class test-bun test-bun-class test-verifiers
...
test-bun:
	COVERAGE_FILE=.coverage.bun coverage run -m pytest tests/package_managers/test_bun.py

test-bun-class:
	COVERAGE_FILE=.coverage.bun.class coverage run -m pytest tests/package_managers/test_bun_class.py
...
coverage-report:
	...
	.coverage.bun .coverage.bun.class \
	.coverage.verifiers
	coverage report
...
.PHONY: checks clean coverage install install-dev test test-bun test-bun-class
