...
directly to determine when a package was created.
"""
...
def get_creation_datetime_utc(package_name: str) -> datetime:
    """
    Return the creation date for the given Bun/npm package.
    ...
    """
    r = requests.get(f"https://registry.npmjs.org/{package_name}", timeout=10)
    r.raise_for_status()
    ...
    if not creation_timestamp:
        raise RuntimeError(
            f"Metadata for Bun package {package_name} missing required fields"
        )
